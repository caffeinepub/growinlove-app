...
export function SpinWheel({ bothCompletedQuiz, combinedQuizState }: SpinWheelProps) {
  ...
  const handleSpin = () => {
    ...
    // Calculate rotation (3-5 full spins + final position)
    const segmentAngle = 360 / loveLanguages.length;
    const targetAngle = selectedIndex * segmentAngle;
    const spins = 3 + Math.floor(Math.random() * 3); // Random 3-5 full rotations
    const finalRotation = rotation + spins * 360 + targetAngle;

    // Set up animation parameters (duration calculated once per spin)
    spinDurationRef.current = 2500 + Math.random() * 1500; // Random duration between 2.5-4 seconds
  ...
  return (
    <Card className="border-2 border-accent/30 shadow-lg bg-gradient-to-br from-accent/5 to-primary/5">
      <CardHeader>
